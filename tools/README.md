## Set up tools for multiple visitor account session usage on Linux
...
' accounts_file.txt > accuunt_file_colon_seperated.txt
```
...
```
# set up to remove all confusing start up nag messages
./setup-vscode-empty.sh 
```

```
# set up to install python extensions
